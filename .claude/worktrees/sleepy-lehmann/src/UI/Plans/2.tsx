        ...
        <div className="flex flex-row max-lg:flex-col items-center justify-center relative gap-8">
            {plans.map((plan, index) => (
                <div key={index} className=" border-2 border-black rounded-xl px-12 py-8 bg-white rounded-xl py-8">
                    ...
                    <h1 className="text-lg  mx-auto border-2 border-black rounded-full px-8 py-1 font-bold text-black text-center mt-4 editable cursor-pointer hover:scale-105 transition-all duration-300">Buy</h1>
                </div>
            ))}
        </div>
    );
